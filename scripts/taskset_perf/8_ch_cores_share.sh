...
Dir_PATH="/home/root/new_testing/tasksetVSparallel/slower_1080p/share"
...
cpu_file="$Dir_PATH/x265_8bit_CPU_${preset}_${count}.csv"
mem_file="$Dir_PATH/x265_8bit_memory_${preset}_${count}.csv"
log_file="$Dir_PATH/x265_8bit_log_${preset}_${count}.txt"
time_file="$Dir_PATH/x265_8bit_time_${preset}_${count}.csv"
...
do
  ...
  # Ensure each process uses a different slice of the video
  x265 --input "$video" --input-res "$input_res" --fps "$fps" --preset "$preset" --output "$output_file" >> "$log_file" 2>&1 &
  pids+=($!)
done
