import os
import csv
import re
import pandas as pd
from collections import defaultdict

def parse_encode_log(log_path):
    fps_values = []
    bitrate_values = []
    ssim = None
    psnr_avg = None
    instructions = None
    elapsed_time = None
    mips = None

    with open(log_path, 'r') as f:
        lines = f.readlines()

    # Original FPS/Bitrate parsing (unchanged)
    for line in lines:
        if line.startswith('encoded'):
            fps_match = re.search(r'(\d+\.\d+) fps', line)
            bitrate_match = re.search(r'(\d+\.\d+) kb/s', line)
            
            if fps_match and bitrate_match:
                fps_values.append(float(fps_match.group(1)))
                bitrate_values.append(float(bitrate_match.group(1)))

    # Original SSIM/PSNR parsing (unchanged)
    for line in lines:
        if 'SSIM Mean Y:' in line:
            ssim_match = re.search(r'SSIM Mean Y: ?([0-9.]+)', line)
            if ssim_match:
                ssim = float(ssim_match.group(1))
        if 'Global PSNR:' in line:
            psnr_match = re.search(r'Global PSNR: ([0-9.]+)', line)
            if psnr_match:
                psnr_avg = float(psnr_match.group(1))
        elif 'PSNR Mean Y:' in line and 'Avg:' in line:
            psnr_match = re.search(r'Avg:([0-9.]+)', line)
            if psnr_match:
                psnr_avg = float(psnr_match.group(1))

    # New MIPS calculation
    for line in lines:
        if 'instructions' in line:
            instr_match = re.search(r'(\d+)\s+instructions', line)
            if instr_match:
                instructions = int(instr_match.group(1))
        if 'seconds time elapsed' in line:
            time_match = re.search(r'(\d+\.\d+)\s+seconds time elapsed', line)
            if time_match:
                elapsed_time = float(time_match.group(1))
    
    if instructions and elapsed_time:
        mips = (instructions / elapsed_time) / 1e6

    return fps_values, bitrate_values, ssim, psnr_avg, mips

def parse_cpu_csv(csv_path):
    try:
        df = pd.read_csv(csv_path)
        return df['cpu_usage'].mean() if 'cpu_usage' in df.columns else None
    except Exception as e:
        print(f"Error reading {csv_path}: {str(e)}")
        return None

def extract_param_value(dir_name):
    return dir_name.split('_')[0]

def get_sort_key(combo):
    gop, bf = combo
    try:
        gop_val = int(gop)
    except ValueError:
        gop_val = 0
    
    try:
        bf_val = int(bf)
    except ValueError:
        bf_val = -1 if bf == 'null' else 0
    
    return (gop_val, bf_val)

def process_directory(root_dir):
    # Maintain original defaultdict structure
    results = defaultdict(lambda: defaultdict(lambda: defaultdict(dict)))
    
    for root, dirs, files in os.walk(root_dir):
        encode_logs = [f for f in files if f.startswith('encode') and f.endswith('.log')]
        if not encode_logs:
            continue

        try:
            path_parts = root.split(os.sep)
            if 'x264' in path_parts:
                base_idx = path_parts.index('x264')
            elif 'x265' in path_parts:
                base_idx = path_parts.index('x265')
            else:
                continue

            video = path_parts[base_idx + 1]
            preset = path_parts[base_idx + 2]
            channels = extract_param_value(path_parts[base_idx + 3])
            gop = extract_param_value(path_parts[base_idx + 4])
            bframes = extract_param_value(path_parts[base_idx + 5])
        except IndexError:
            continue

        # Original metric collection with MIPS addition
        total_fps = []
        total_bitrate = []
        ssim_values = []
        psnr_values = []
        mips_values = []
        
        for log_file in encode_logs:
            fps, bitrate, ssim, psnr, mips = parse_encode_log(os.path.join(root, log_file))
            total_fps.extend(fps)
            total_bitrate.extend(bitrate)
            if ssim is not None: ssim_values.append(ssim)
            if psnr is not None: psnr_values.append(psnr)
            if mips is not None: mips_values.append(mips)

        avg_fps = sum(total_fps)/len(total_fps) if total_fps else None
        avg_bitrate = sum(total_bitrate)/len(total_bitrate) if total_bitrate else None
        avg_ssim = sum(ssim_values)/len(ssim_values) if ssim_values else None
        avg_psnr = sum(psnr_values)/len(psnr_values) if psnr_values else None
        avg_mips = sum(mips_values)/len(mips_values) if mips_values else None
        cpu_avg = parse_cpu_csv(os.path.join(root, 'cpu.csv'))

        # Maintain original data structure format
        results[video][preset][channels][(gop, bframes)] = {
            'FPS': avg_fps,
            'Bitrate': avg_bitrate,
            'SSIM': avg_ssim,
            'PSNR': avg_psnr,
            'MIPS': avg_mips,
            'CPU Usage': cpu_avg
        }

    return results

def save_to_csv(data, output_file):
    if not data:
        print("No data to save")
        return

    # Original CSV structure with MIPS column addition
    unique_combos = set()
    for video_data in data.values():
        for preset_data in video_data.values():
            for channels_data in preset_data.values():
                unique_combos.update(channels_data.keys())

    sorted_combos = sorted(unique_combos, key=get_sort_key)
    presets_order = ['ultrafast', 'veryfast', 'fast', 'medium', 'slow', 'slower']
    
    # Modified header with MIPS column
    fieldnames = ['Video', 'Preset', 'Channels']
    for gop, bf in sorted_combos:
        fieldnames.extend([
            f'FPS ({gop},{bf})',
            f'Bitrate ({gop},{bf})',
            f'SSIM ({gop},{bf})',
            f'PSNR ({gop},{bf})',
            f'MIPS ({gop},{bf})',
            ''  # Maintain original empty column
        ])
    if fieldnames[-1] == '': fieldnames.pop()

    # Original CSV writing logic
    with open(output_file, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(fieldnames)

        def video_sort_key(v):
            match = re.search(r'_(\d+)x(\d+)_', v)
            return (int(match.group(1)), int(match.group(2))) if match else (0, 0)

        for video in sorted(data.keys(), key=video_sort_key):
            video_written = False
            
            for preset in presets_order:
                if preset not in data[video]: continue
                
                for channels in sorted(data[video][preset].keys(), key=int):
                    row = [video if not video_written else '', preset, channels]
                    video_written = True
                    
                    for combo in sorted_combos:
                        metrics = data[video][preset][channels].get(combo, {})
                        row.extend([
                            metrics.get('FPS', ''),
                            metrics.get('Bitrate', ''),
                            metrics.get('SSIM', ''),
                            metrics.get('PSNR', ''),
                            metrics.get('MIPS', '')
                        ])
                        if combo != sorted_combos[-1]:
                            row.append('')
                    
                    writer.writerow(row)
            
            writer.writerow([])

if __name__ == '__main__':
    root_directory = '/home/mcw/Downloads/Video/qcs_folders/8bit_4k_1ch_ramdisk_encode_perf_stat/optimised_encoding/x265/'
    output_csv = '/home/mcw/Downloads/Video/qcs_folders/8bit_4k_1ch_ramdisk_encode_perf_stat/optimised_encoding/x265/x265_MIPS.csv'
    
    metrics_data = process_directory(root_directory)
    save_to_csv(metrics_data, output_csv)
    print(f"Report generated: {output_csv}")
