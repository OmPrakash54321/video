...
for codec in "${encoders[@]}"; do
    for video in "${videos[@]}"; do
        ...
        # This is a temporary block that has to generalised
	if [ "$fs" = "3840x2160" ]; then
            bitrate=4200
        elif [ "$fs" = "1920x1080" ]; then
            bitrate=1400
        elif [ "$fs" = "1280x720" ]; then
            bitrate=700
        else
            bitrate=179
        fi
                ...
                for keyint in "${keyint_values[@]}"; do
                    for bframes in "${bframes_values[@]}"; do
                        ...
                        for j in $(seq 1 $ch); do
                            ...
                            if [ "$codec" = "x264" ]; then
                               perf record -o "$log_dir/perf_$j.data" -F 99 -g -e cpu-cycles,instructions,cycles,cache-misses,branch-misses,context-switches,L1-dcache-loads,L1-dcache-load-misses,LLC-loads,LLC-load-misses -- x264 --input-res "$fs" --fps "$fps" --preset "$preset" \
                               ...
                               perf record -o "$log_dir/perf_$j.data" -F 99 -g -e cpu-cycles,instructions,cycles,cache-misses,branch-misses,context-switches,L1-dcache-loads,L1-dcache-load-misses,LLC-loads,LLC-load-misses -- x265 --input-res "$fs" --fps "$fps" --input "$INPUT_DIR/$video" \
