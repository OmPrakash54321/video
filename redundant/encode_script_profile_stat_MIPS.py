...
def parse_encode_log(log_path):
    ...
    for line in lines:
        if 'instructions' in line:
            instr_match = re.search(r'^\s*(\d+)\s+instructions\b', line.strip())
            if instr_match:
                instructions = int(instr_match.group(1))
        if 'seconds time elapsed' in line:
            time_match = re.search(r'^\s*(\d+\.\d+)\s+seconds time elapsed', line.strip())
            if time_match:
                elapsed_time = float(time_match.group(1))

    # Calculate MIPS if both values found
...
if __name__ == '__main__':
    root_directory = '/home/mcw/Downloads/Video/qcs_folders/8bit_4k_1ch_ramdisk_encode_perf_stat/optimised_encoding/x264'
    output_csv = '/home/mcw/Downloads/Video/qcs_folders/8bit_4k_1ch_ramdisk_encode_perf_stat/optimised_encoding/x264/x264_MIPS.csv'
