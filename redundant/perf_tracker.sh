...
# Directory paths
INPUT_DIR="/home/mcw/Downloads/Video/script_testing/video_source"
OUTPUT_BASE="/home/mcw/Downloads/Video/script_testing/output"

# Configuration arrays
declare -a encoders=("x264" "x265")
declare -a presets=("ultrafast" "superfast" "veryfast" "faster" "fast" "medium" "slow" "slower" "veryslow" "placebo")
declare -a ch_values=("1" "4" "8" "16")
        ...
        for preset in "${presets[@]}"; do
            for ch in "${ch_values[@]}"; do
                ...
                for j in $(seq 1 $ch); do
                    ...
                    output_file="$current_dir/${video_name}_${j}.${codec##*x}"
                    ...
                    if [ "$codec" = "x264" ]; then
                        x264 --preset "$preset" \
                            ...
                            --ssim \
                            -o "$output_file" \
                            "$INPUT_DIR/$video" > "$log_file" 2>&1 &
                    elif [ "$codec" = "x265" ]; then
                        x265 --input "$INPUT_DIR/$video" \
                            --preset "$preset" \
                            --psnr \
                            --ssim \
