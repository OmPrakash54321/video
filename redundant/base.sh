...
# Directory paths
INPUT_DIR="/home/mcw/Downloads/Video/x264_scripts_mod"       # Replace with the directory containing your raw YUV input videos

# Define input videos, resolutions, and presets
declare -a videos=("out_ff.y4m")
declare -a resolutions=("1920x1080")
...
OUTPUT_DIR="/home/mcw/Downloads/Video/x264_scripts_mod/outputx264/ch_${ch}"     # Replace with the directory where encoded outputs will be stored
...
for i in "${!videos[@]}"; do
    ...
    for preset in "${presets[@]}"; do
        ...
        for ch in "${ch[@]}"; do
            # Run x264 command in parallel (background) for $ch jobs at a time
            for j in $(seq 1 $ch); do
                ...
                # If the number of parallel jobs reaches the limit ($ch), wait for them to complete
                if [ "${#jobs[@]}" -ge $ch ]; then
